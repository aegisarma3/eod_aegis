...
if (isServer) then
{
	...
	if ((count _syncObjs) == 0) exitWith {diag_log "Log: [EOD_AMBIED] Shutting down. No units were synchronized to the module."};
	...
	reezo_eod_module_ambied_loop = compile preprocessFileLineNumbers "x\eod\addons\eod\reezo_eod_module_ambied_loop.sqf";
	...
	for "_i" from 0 to (count _syncObjs - 1) do
	{
		...
		{
			[_this, _thisUnit] spawn reezo_eod_module_ambied_loop;
			_thisUnit setVariable ["reezo_eod_synced_ambied", true];
		};
	};
