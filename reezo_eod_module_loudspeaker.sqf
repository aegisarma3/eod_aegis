...
if (isServer) then
{
	...
	{
		[-1, {reezo_eod_action_loudspeaker = (_this select 0) addAction ['<t color="#FF9900">'+"Loudspeaker (Evacuate Civilians)"+'</t>', "x\eod\addons\eod\reezo_eod_action_loudspeaker_evacuate.sqf", [_this select 0,_this select 1,_this select 2,_this select 3], 0, false, true, "",""]}, [_x, _range, _probability, _interval]] call CBA_fnc_globalExecute;
	} forEach _syncObjs;
